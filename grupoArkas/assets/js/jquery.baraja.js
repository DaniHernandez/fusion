...
;( function( $, window, undefined ) {
	...
	$.Baraja.prototype = {
		...
		_navigate : function( dir ) {
			...
			this._applyTransition( $item, { transform : 'translate(' + translation + 'px) rotate(' + rotation + 'deg)' ,  height : '600px', width : '220px' }, function() {
				...
				self._applyTransition( $item, { transform : 'translate(0px) rotate(0deg)' }, function() {
					$(".baraja p").css({transition: 'opacity 1s linear'});
					$(".baraja p").css({opacity : 1})
					$item.off( self.transEndEventName );
		...
		_move2front : function( $item ) {
			...
			setTimeout( function() {
				...
				self._applyTransition( $item, { transform : 'none', height : '600px', width : '220px', opacity : 1 }, function() {
					$(".baraja p").css({transition: 'opacity 0.5s ease-in'});
					$(".baraja p").css({opacity : 1})
					$item.off( self.transEndEventName );
					self.isAnimating = false;
		...
		_close : function( callback, $item ) {
			...
			this._applyTransition( $items, { transform : 'none'}, function() {
				...
				setTimeout(function() {
					...
					self._setTransition( $items, 'all', self.options.speed, 'ease-in' );
					$items.css( {height : '600px', width : '220px'} );
					$(".baraja p").css({transition: 'opacity 0.5s ease-in 0.2s'});
					$(".baraja p").css({opacity : 1})
					if( callback ) {
						callback.call();
					}
		...
		_fan : function( settings ) {
			...
			this.$items.each( function( i ) {
				...
				self._applyTransition( $el, { transform : 'translate(' + position + 'px) rotate(' + angle + 'deg)', height : '310px', width : '200px', }, function() {
					++cnt;
